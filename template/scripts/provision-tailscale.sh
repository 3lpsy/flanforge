...
[[ "${extra_args_string}" != *$'\n'* && "${extra_args_string}" != *$'\r'* ]] \
  || { echo "TAILSCALE_EXTRA_ARGS must be a single line" >&2; exit 1; }
[ "${#extra_args_string}" -le 4096 ] \
  || { echo "TAILSCALE_EXTRA_ARGS must not exceed 4096 characters" >&2; exit 1; }
...
if [[ "${extra_args_string}" =~ [^[:space:]] ]]; then
