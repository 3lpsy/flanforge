...
# Accessibility, screen recording, event post, and Apple events for the job
# account. Disabling this leaves jobs unable to capture the host screen,
# synthesize input, or script another application.
variable "privacy_grants_enabled" {
  type    = bool
  default = true
}
...
locals {
  provisioning_environment = [
    ...
    "PRIVACY_GRANTS_ENABLED=${var.privacy_grants_enabled}",
...
build {
  ...
  provisioner "file" {
    source      = "${path.root}/../shared/dependencies/routes.sh"
    destination = "/tmp/flanforge-dependency-routes.sh"
  }

  provisioner "file" {
    source      = "${path.root}/../shared/dependencies/chilled-proxy.init.gradle"
  ...
  provisioner "shell" {
    scripts = [
      ...
      "scripts/provision-runner-privacy.sh",
