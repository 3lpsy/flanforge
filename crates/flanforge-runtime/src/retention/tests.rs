...
#[test]
fn only_retirement_and_publication_prove_the_live_image_was_removed() {
    for untouched in [
        ...
        assert!(!is_live_image_removed(untouched), "{untouched:?}");
    }
    assert!(is_live_image_removed(RetentionPhase::Retire));
    assert!(is_live_image_removed(RetentionPhase::Promote));
}
