...
pub(super) async fn firewall_gate(
    config_path: &Path,
    target: &Target,
    plan: Plan,
    ...
    if plan.is_granting_firewall && matches!(&state, Ok(read) if !read.is_satisfied()) {
        if let Err(error) = grant_firewall(config_path, &target.binary, overrides).await {
            return Gate::new(
                Gated::Firewall,
                GateState::Missing,
...
async fn grant_firewall(
    ...
    if elevate::is_root().await? {
        ...
        elevate::elevate(config_path, binary, overrides).await
    }
}
...
pub(super) async fn tcc_gates(
    ...
    let probes = match &target.probe_via {
        Some(binary) => {
            delegated_probes(binary, config_path, bound, plan.is_prompting, overrides).await
        }
        None => run_probes(config_path, bound, overrides).await,
    };
...
fn volume_gate(
    ...
    let tart = config.map_or_else(
        || "the configured runtime.backend.path".to_owned(),
        |config| {
            config.runtime.tart().map_or_else(
                || "the Tart executable".to_owned(),
                |tart| tart.path.display().to_string(),
            )
        },
    );
