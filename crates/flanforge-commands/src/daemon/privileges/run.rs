...
pub(crate) async fn priv_gates(
    ...
    if arguments.probe {
        return print_probes(config_path, arguments.prompt, overrides).await;
    }
    ...
    let mut gates = vec![firewall_gate(config_path, &target, plan, overrides).await];
    gates.extend(tcc_gates(config_path, &target, plan, overrides).await);
    for gate in &gates {
        gate.print();
    }
...
async fn print_probes(
    ...
    let probes = run_probes(config_path, bound, overrides).await;
