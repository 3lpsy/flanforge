...
use flanforge_cli::ReaperCommand;
...
pub async fn run_reaper_command(
    path: &Path,
...
fn print_report(report: &SweepReport, is_delete: bool) {
    ...
    // Only the populated case prints: a host with nothing unageable stays quiet.
    if !report.unaged.is_empty() {
        println!(
            "never collectable (no determinable age): {}",
            report.unaged.join(", ")
        );
    }
    for image in &report.retired_images {
        println!(
            "  warm generation {} of {} ({}s): {}{}",
            image.generation,
            image.profile,
            image.age_seconds,
            image.reason,
            if image.is_deleted { "" } else { " (retained)" }
        );
    }
...
mod tests {
    ...
    use flanforge_cli::{ReaperCommand, ReaperRunArgs};

    use super::run_reaper_command;

    fn command(delete: bool) -> ReaperCommand {
        ReaperCommand::Run(ReaperRunArgs { delete })
    }
    ...
    async fn run_against_a_fake_service(delete: bool) -> String {
        ...
        run_reaper_command(&path, command(delete), &ConfigOverrides::default())
            .await
