...
usage() {
  printf '%s\n' \
    "Usage:" \
    "  ${0##*/} [--dry-run] [forgejo-runner-darwin-arm64]" \
    "  ${0##*/} -h | --help | help" \
    "" \
    ...
    "" \
    "Settings below may be added to the project-root .env; required values" \
    "are identified in their descriptions." \
    "That file is sourced as trusted shell syntax before options are resolved." \
    "See .env.example for a complete copyable configuration." \
    "" \
    "Environment:" \
...
if [ -f "${env_file}" ]; then
  echo "==> loading build settings from ${env_file}"
  set -a
...
base_image="${TART_BASE_IMAGE:-ghcr.io/cirruslabs/macos-tahoe-xcode:latest}"
...
[[ "${tailscale_extra_args}" != *$'\n'* && "${tailscale_extra_args}" != *$'\r'* ]] \
  || die "FLANFORGE_TAILSCALE_EXTRA_ARGS must be a single line"
[ "${#tailscale_extra_args}" -le 4096 ] \
  || die "FLANFORGE_TAILSCALE_EXTRA_ARGS must not exceed 4096 characters"
...
if [ -n "${services_root_domain}" ]; then
  [[ "${services_root_domain}" =~ ^[A-Za-z0-9]([A-Za-z0-9.-]*[A-Za-z0-9])?$ ]] \
...
if [ "${generated_guest_key}" = true ]; then
  ...
  echo "    config path: guest.ssh_identity_file = \"${default_guest_identity}\""
